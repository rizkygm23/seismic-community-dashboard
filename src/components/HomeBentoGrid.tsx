...
const LeaderboardVisual = ({ data }: { data: HomeBentoData['leaderboard'] }) => {
    const tooltipItems = data.map((user, i) => ({
        id: i + 1,
        name: user.username,
        designation: `${user.score.toLocaleString()} Contribution`,
...
// 4. Explore: Role Holders
const ExploreVisual = ({ data }: { data: HomeBentoData['explore'] }) => {
    return (
        <div className="flex flex-1 w-full h-full min-h-[6rem] bg-gradient-to-br from-neutral-900 to-neutral-800 border border-neutral-800 rounded-lg p-6 flex-col justify-center items-center relative overflow-hidden group">
            <div className="absolute inset-0 bg-dot-white/[0.1] opacity-50" />
            ...
            >
                <div className="text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-neutral-500">
                    {data.roleHolderCount}
                </div>
                <div className="text-sm text-neutral-500 mt-2 font-medium uppercase tracking-widest">Role Holders</div>
            </motion.div>
        </div>
    );
};

// 5. Promotion: Top 3 Users Highlights
const PromotionVisual = ({ data }: { data: HomeBentoData['promotion'] }) => {
    const tooltipItems = data.users.map((user, i) => ({
        id: i + 1,
        name: user.username,
        designation: `Magnitude ${Math.floor(user.roleMagnitude)}`,
        image: user.avatar_url || "/avatar-placeholder.png",
    }));

    return (
        <div className="flex flex-1 w-full h-full min-h-[6rem] bg-gradient-to-bl from-neutral-900 to-neutral-800 border border-neutral-800 rounded-lg p-5 flex-col items-center justify-center relative overflow-hidden">
            {data.hasPromotion && data.users.length > 0 ? (
                <div className="flex flex-row items-center justify-center w-full relative z-10">
                    <AnimatedTooltip items={tooltipItems} />
                </div>
            ) : (
                <div className="flex flex-col items-center justify-center h-full text-center p-4 relative z-10">
                    ...
                    <p className="text-sm text-neutral-400">No promotions this week yet.</p>
                </div>
            )}
        </div>
    );
...
const items = (data: HomeBentoData) => [
    ...
    },
    {
        ...
        description: "",
        header: <PromotionVisual data={data.promotion} />,
        className: "md:col-span-1",
    ...
    {
        title: "Role Holders",
        description: "Community magnitude distribution",
        header: <ExploreVisual data={data.explore} />,
        className: "md:col-span-1",
